...
<body>
  ...
  <nav class="top-nav">
    ...
    <div class="nav-links">
      ...
      <a href="#ai-ideas" class="nav-link">AI Ideas</a>
    </div>
    <div class="nav-right">
...
@media(min-width:700px){.ai-grid{grid-template-columns:1fr 1fr;}}
.ai-card{
  display:flex;gap:14px;align-items:flex-start;
  padding:20px;border-radius:var(--radius-lg);
  background:linear-gradient(135deg,rgba(255,143,92,0.06),var(--glass));
...
.ai-card:hover{
  ...
  border-color:rgba(255,143,92,0.30);
  box-shadow:0 8px 32px rgba(255,143,92,0.10);
}
.ai-card-icon{font-size:28px;flex-shrink:0;line-height:1;margin-top:2px;}
...
.ai-card-desc{
  font-size:13.5px;color:var(--fg-3);line-height:1.65;
}
.ai-footer{
  text-align:center;margin-top:20px;
  font-family:var(--mono);font-size:11px;color:var(--fg-5);
  letter-spacing:0.05em;
}
.ai-footer a{color:var(--hot-2);text-decoration:none;}
...
// ══════════════════════════════════════
// AI CREATIVE IDEAS SECTION
// ══════════════════════════════════════

function renderAISection(data: ProjectSnapshot): string {
  const suggestions = data.aiSuggestions;
  if (!suggestions || suggestions.length === 0) return '';

  const cards = suggestions.map((s, i) => {
    const delay = i * 0.08;
    return `<div class="ai-card" style="animation-delay:${delay}s">
      <div class="ai-card-icon">${esc(s.icon)}</div>
      <div class="ai-card-body">
        <div class="ai-card-title">${esc(s.title)}</div>
  ...
  return `<div class="section-divider"></div>
<section class="section" id="ai-ideas">
  <div class="section-head">
    <div class="section-badge hot">✨ AI Creative Ideas</div>
    <h2 class="section-title">AI-Powered <span class="grad">Inspiration</span></h2>
    <p class="section-desc">Personalized creative suggestions generated by AI based on your project's unique characteristics</p>
  </div>
  <div class="ai-grid">
    ${cards}
  </div>
  <div class="ai-footer">Generated by AI · <a href="https://www.douglasfugazi.co" target="_blank">Project Snapshot AI</a></div>
</section>`;
}
